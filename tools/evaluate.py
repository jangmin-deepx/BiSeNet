...
class MscEvalV0(object):
    ...
    def __call__(self, net, dl, n_classes):
        ...
        for i, (imgs, label) in diter:
            ...
            size = label.size()[-2:]
            probs = (
                torch.zeros((N, n_classes, H, W), dtype=torch.float32).cuda().detach()
            )
            for scale in self.scales:
                sH, sW = int(scale * H), int(scale * W)
                sH, sW = get_round_size((sH, sW))
                im_sc = F.interpolate(
                    imgs, size=(sH, sW), mode="bilinear", align_corners=True
                )
            ...
            hist += torch.bincount(
                label[keep] * n_classes + preds[keep], minlength=n_classes**2
...
class MscEvalCrop(object):
    def __init__(
        ...
        self.cropsize = (
            cropsize if isinstance(cropsize, (list, tuple)) else (cropsize, cropsize)
        )
...
def eval_model(cfg, net):
    ...
    # diter = enumerate(dl)
    # idx, (img, label) = next(diter) # H=1024, W=2048
    net.eval()

    heads, mious = [], []
    logger = logging.getLogger()

    single_scale = MscEvalV0((1.0,), False)
