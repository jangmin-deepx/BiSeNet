...
def save_func(inpth, outpth, out_q):
    ...
    video_writer = cv2.VideoWriter(
        outpth, cv2.VideoWriter_fourcc(*"mp4v"), fps, (int(width), int(height))
    )
...
def infer_batch(frames):
    ...
    frames = F.interpolate(
        frames, size=(768, 768), mode="bilinear", align_corners=False
...
if __name__ == "__main__":
    ...
    out_worker = Process(target=save_func, args=(args.input, args.output, out_q))
