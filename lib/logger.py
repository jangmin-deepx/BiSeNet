...
def setup_logger(name, logpth):
    ...
    try:
        logging.basicConfig(
            level=log_level, format=FORMAT, filename=logfile, force=True
        )
    except Exception:
...
def print_log_msg(
    it, max_iter, lr, time_meter, loss_meter, loss_pre_meter, loss_aux_meters
):
    t_intv, eta = time_meter.get()
    loss_avg, _ = loss_meter.get()
    loss_pre_avg, _ = loss_pre_meter.get()
    loss_aux_avg = ", ".join(
        ["{}: {:.4f}".format(el.name, el.get()[0]) for el in loss_aux_meters]
    )
    msg = ", ".join(
        [
            "iter: {it}/{max_it}",
